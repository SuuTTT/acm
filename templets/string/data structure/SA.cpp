//index_0 suffix(len)代表空串。 rmq时要l++
#include <bits/stdc++.h>
using namespace std;
...
int main(){
    ...
    while(cin>>s){
        ...
        rmq_init(len,height);
         ...
         for(int i=0;i<=len;i++)debug(height[i]),debug(Rank[i]),debug(sa[i]);
        int ans=0,anspos=-1,anslen=0;//ans repetation
        rep(i,1,len/2)for(int j=0;j+i<len;j+=i){
            if(a[j]!=a[j+i])continue;
            int tmp = rmq(Rank[j],Rank[j+i]);
            debug(tmp),debug(Rank[j]),debug(Rank[j+i]);
            int k=j-1,p = j;//k pointer, p anspos of current loop
            while(k>=0&&k+i>j and a[k]==a[k+1]){
                tmp++;
                if(tmp%i==0){// ans++
                    p=k;
