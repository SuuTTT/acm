...
支持多项式大多数操作， `poly<T>` 的系数类为 `T`.

支持四则运算 `+`, `-`, `*`, `%` and `/`, `%` and `/` 代表除和取余。

模质数 `m`意义下的多项式用 `modular<m>` 表示。

其它函数：

- `deriv()`: 求微分 $P'(x)$ of $P(x)$.
- `integr()`: 求积分 $Q(x) = \int P(x)$ of $P(x)$ such that $Q(0)=0$.
- `inv(size_t n)`: 求逆的前n个系数 $P^{-1}(x)$ in $O(n \log n)$.
- `log(size_t n)`: 求ln的前n个系数 $\ln P(x)$ in $O(n \log n)$.
- `exp(size_t n)`: 求exp的前n个系数 $\exp P(x)$ in $O(n \log n)$.
- `pow(size_t k, size_t n)`: 求幂的前n个系数 $P^{k}(x)$ in $O(n \log nk)$.
- `deg()`: 返回次数 $P(x)$.
- `lead()`: 返回最高次项 $x^{\deg P(x)}$.
- `resultant(poly<T> a, poly<T> b)`: 合矢量 of $a$ and $b$ in $O(|a| \cdot |b|)$.
- `bpow(T x, size_t n)`: 计算 $x^n$.
- `bpow(T x, size_t n, T m)`:  计算 $x^n \pmod{m}$.
- `chirpz(T z, size_t n)`:  计算 $P(1), P(z), P(z^2), \dots, P(z^{n-1})$ in $O(n \log n)$.
- `vector<T> eval(vector<T> x)`:  计算$P(x_1), \dots, P(x_n)$ in $O(n \log^2 n)$.
- `poly<T> inter(vector<T> x, vector<T> y)`: 插值出一个多项式，根据一组 $P(x_i) = y_i$ in $O(n \log^2 n)$.
- And some more, feel free to explore the code!
- [函数具体实现细节](https://www.cnblogs.com/SuuT/p/11352944.html)
- [洛谷日报](https://www.luogu.org/blog/user7035/duo-xiang-shi-zong-jie)
